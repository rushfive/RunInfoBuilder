...
namespace R5.RunInfoBuilder.Help
{
	...
	internal class ProgramDescriptionConfig : HelpFormatConfig
	{
		...
		protected ProgramDescriptionConfig(
			PaddingConfig padding,
			string description,
			int lineBreaksCount) 
			: base(paddingSpaces, paddingCustom)
		{
		}
	...
	{
		private string _paddingSpaces { get; }

		internal virtual int PaddingSpacesLength { get; } = 4;
		internal virtual string PaddingCustom { get; } = string.Empty;



		internal virtual bool DisplayProgramDescription { get; } = true;
		internal virtual int ProgramDescriptionLineBreaks { get; } = 1;

		internal virtual int SectionLineBreaks { get; } = 1;

		

		// todo: maybe break these into separate classes
		internal virtual CommandsFormatConfig Commands { get; set; } = new CommandsFormatConfig();
	...
	internal abstract class HelpFormatConfig
	{
		...
		protected static readonly string Tab = "\t";
		protected static readonly string Empty = string.Empty;
